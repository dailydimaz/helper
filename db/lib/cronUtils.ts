...
export const setupJobFunctions = async () => {
  // Ensure HMAC secret exists in our secrets table (lightweight version - no vault)
  const hmacSecret = await getOrCreateSecret("JOBS_HMAC");

  await db.execute(
    sql.raw(`
      create or replace function call_job_endpoint(job_body text, queue_message_id text) returns text as $$
      declare
        endpoint_url text := '${env.NODE_ENV === "development" ? "http://localhost:3000" : env.NEXT_PUBLIC_APP_URL}/api/job';
        hmac_secret text;
        timestamp_str text;
        hmac_payload text;
        response text;
      begin
        -- For lightweight version, get the secret from our secrets table
        select '${hmacSecret}' into hmac_secret;
