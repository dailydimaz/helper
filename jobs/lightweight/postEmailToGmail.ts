...
export const postEmailToGmail = async (payload: { messageId: number }) => {
  const { messageId: emailId } = payload;
  ...
  if (!email) {
    console.log(`Email ${emailId} not found or not in queueing status`);
    return;
  }
  ...
  try {
    ...
    const sentEmail = await getMessageMetadataById(gmailService, response.data.id!);
    ...
    console.log(`Successfully sent email ${emailId} to Gmail`);
  } catch (e) {
